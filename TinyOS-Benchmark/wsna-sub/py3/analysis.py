...
#==============================================================================
    ...
    ''' 获得每个时间窗口的状态链，foo为list '''
    #node_3_wins = nodes[node_No].split_timewindow_logcalls(time_gap)
    #node_4_wins = nodes['node_4'].split_timewindow_logcalls(time_gap)

    #node_2_wins = nodes['node_2'].split_timewindow_logcalls(time_gap)
    #m3_1 = get_prob_mat([i for i in node_3_wins if node_3_wins.index(i)%2 != 0])
    #m3_2 = get_prob_mat([i for i in node_3_wins if node_3_wins.index(i)%2 == 0])
    #m3_1 = get_prob_mat(node_3_wins[11:len(node_3_wins)/2]) # 取前1/3  
    #m3_2 = get_prob_mat(node_3_wins[-len(node_3_wins)/2:]) # 取后1/3 
    
    #m3_1 = get_prob_mat(map(clear_few_task , node_1_wins[: 1])) # 取前1/3 并去掉频数少的任务 
    #m3_2 = get_prob_mat(map(clear_few_task , node_1_wins[1: 3])) # 取后1/3 
    
    #norm_mode = get_prob_mat(node_3_wins[50: 85])
    
    #m4 = get_prob_mat([i for i in node_4_wins if node_4_wins.index(i)%2 != 0])
    ...
    # 根据弧度计算角度
    tmp_acos = lambda x: (math.acos(x)/(2*math.pi))*360.0
    ''' 分别计算每个任务转移概率的夹角 '''
    def get_mat_anyvector_angle(m3_1, m3_2):
        ...
        """
        angle = dict()
        for e1, e2 in zip(m3_1, m3_2):
            ''' 断言防止m3_1与m3_2中的元素不一样导致不是对应的向量在比较 '''
            assert e1==e2, 'matrix must be same kind:' + e1 + ', ' + e2
            task_name, ang = e1, tmp_acos(vector_angle(zip(m3_1[e1].values(), m3_2[e2].values())))
            angle[task_name]  = ang
            #print task_name, ang
        return angle
...
#==============================================================================
    ...
    def split_chunks(arr, m):
        ...
        n = int(math.ceil(len(list(arr)) / float(m)))
        print('-->', len(arr))
        return [arr[i:i + n] for i in range(0, len(list(arr)), n)]
...
#==============================================================================
    ...
    for i in range(1): 
        ...
        train_set = split_chunks(list(node_1_wins), len(list(node_1_wins)))[:int(round(len(list(node_1_wins))/5.0))]
        ...
        for i in split_chunks(node_1_wins, len(node_1_wins))[0:len(node_1_wins)]:
            c += 1
            foo = verify_mode(i, modes, a+2)
            print(c, '--\t', foo)
