...
#==============================================================================
    ...
    #labels =  ['Time window ' + str(i+1) for i in range(10)]
    labels =  ['1st time window', '2nd time window']\
    + [str(i+1) + 'th time window' for i in range(2, 10)]
    cache_argv = list()
    bar_width = 0.5
...
#     for k, v in kw.items():
#         if not cache_argv:
#             plt.barh(v, [i for i in range(len(v))], 
#                 label=k, alpha=0.5, color=colors[0])
#         else:
#             plt.barh(v, [sum(e) for e in zip(*cache_argv)], 
#                      label=k, alpha=0.5,
#                     color=colors[len(cache_argv)%len(colors)])
#         plt.yticks([i+1 for i in range(len(v))], labels, fontsize =10, fontproperties=myfont) 
#         cache_argv.append(v)
# 	
#==============================================================================
    ...
    plt.ylim([min(tick_pos)-bar_width, max(tick_pos)*37])
    plt.grid()
